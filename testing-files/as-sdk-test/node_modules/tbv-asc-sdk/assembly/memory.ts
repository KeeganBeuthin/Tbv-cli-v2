...
export function readString(ptr: usize): string {
  consoleLog(`readString called with ptr: ${ptr}`);
  if (ptr === 0) {
    consoleLog("Error: Null pointer passed to readString");
    return "";
  }
  const len = getStringLen(ptr);
  consoleLog(`String length: ${len}`);
  if (len === 0) {
    consoleLog("Warning: Empty string");
    return "";
  }
